		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAWSS3BucketObjectExists("opentelekomcloud_s3_bucket_object.object", &rObj),
				),
			},
			resource.TestStep{
				Config: conf,
		...
		Steps: []resource.TestStep{
			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestMatchResourceAttr("data.opentelekomcloud_s3_bucket_object.obj", "last_modified",
						regexp.MustCompile("^[a-zA-Z]{3}, [0-9]+ [a-zA-Z]+ [0-9]{4} [0-9:]+ [A-Z]+$")),
					resource.TestCheckResourceAttr("data.opentelekomcloud_s3_bucket_object.obj", "body", "Keep Calm and Carry On"),
				),
			},
		},
	})
...
resource "opentelekomcloud_kms_key" "example" {
...
resource "opentelekomcloud_s3_bucket_object" "object" {
	...
	content_type = "text/plain"
	kms_key_id = "${opentelekomcloud_kms_key.example.arn}"
}
`, randInt, randInt)
