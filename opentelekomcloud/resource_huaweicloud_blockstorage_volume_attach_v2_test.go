		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckBlockStorageVolumeAttachV2Exists("opentelekomcloud_blockstorage_volume_attach_v2.va_1", &va),
				),
			},
		},
	})
...
func testAccCheckBlockStorageVolumeAttachV2Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "opentelekomcloud_blockstorage_volume_attach_v2" {
			continue
		}
