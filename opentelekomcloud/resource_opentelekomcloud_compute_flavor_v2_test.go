		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2FlavorExists("opentelekomcloud_compute_flavor_v2.flavor_1", &flavor),
				),
			},
		},
	})
...
func testAccCheckComputeV2FlavorDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "opentelekomcloud_compute_flavor_v2" {
			continue
		}
...
func testAccComputeV2Flavor_basic(flavorName string) string {
	return fmt.Sprintf(`
    resource "opentelekomcloud_compute_flavor_v2" "flavor_1" {
      name = "%s"
      ram = 2048
      vcpus = 2
      disk = 5
