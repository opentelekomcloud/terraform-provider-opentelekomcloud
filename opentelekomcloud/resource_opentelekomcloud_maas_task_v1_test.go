		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMaasTaskV1Exists("opentelekomcloud_maas_task_v1.task_1"),
					resource.TestCheckResourceAttr("opentelekomcloud_maas_task_v1.task_1", "description", "migration task"),
				),
			},
		},
	})
...
func testAccCheckMaasTaskV1Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "opentelekomcloud_maas_task_v1" {
			continue
		}
