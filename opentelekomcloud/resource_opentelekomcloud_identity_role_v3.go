...
func buildIdentityRoleV3CreateParameters(opts map[string]interface{}, arrayIndex map[string]int) (interface{}, error) {
	...
	empty, err := isEmptyValue(reflect.ValueOf(descriptionProp))
	if err != nil {
		return nil, err
	}
	if !empty {
	...
	empty, err = isEmptyValue(reflect.ValueOf(displayNameProp))
	if err != nil {
		return nil, err
	}
	if !empty {
	...
	empty, err = isEmptyValue(reflect.ValueOf(policyProp))
	if err != nil {
		return nil, err
	}
	if !empty {
	...
	empty, err = isEmptyValue(reflect.ValueOf(typeProp))
	if err != nil {
		return nil, err
	}
	if !empty {
