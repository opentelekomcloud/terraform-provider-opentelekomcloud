			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"opentelekomcloud_objectstorage_container_v1.container_1", "name", "container_1"),
					resource.TestCheckResourceAttr(
						"opentelekomcloud_objectstorage_container_v1.container_1", "content_type", "application/json"),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"opentelekomcloud_objectstorage_container_v1.container_1", "content_type", "text/plain"),
				),
			},
		},
	})
...
func testAccCheckObjectStorageV1ContainerDestroy(s *terraform.State) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating OpenTelekomCloud object storage client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "opentelekomcloud_objectstorage_container_v1" {
			continue
		}
...
resource "opentelekomcloud_objectstorage_container_v1" "container_1" {
  name = "container_1"
  metadata {
    test = "true"
  }
