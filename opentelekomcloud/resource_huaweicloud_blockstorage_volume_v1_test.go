		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckBlockStorageV1VolumeExists("opentelekomcloud_blockstorage_volume_v1.volume_1", &volume),
					testAccCheckBlockStorageV1VolumeMetadata(&volume, "foo", "bar"),
					resource.TestCheckResourceAttr(
						"opentelekomcloud_blockstorage_volume_v1.volume_1", "name", "volume_1"),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckBlockStorageV1VolumeMetadata(&volume, "foo", "bar"),
					resource.TestCheckResourceAttr(
						"opentelekomcloud_blockstorage_volume_v1.volume_1", "name", "volume_1-updated"),
				),
			},
		},
	})
...
func testAccCheckBlockStorageV1VolumeDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "opentelekomcloud_blockstorage_volume_v1" {
			continue
		}
