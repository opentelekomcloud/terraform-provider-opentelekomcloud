...
func resourceLBMonitorV1Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating OpenTelekomCloud LB Monitor: %s", err)
	}
	log.Printf("[INFO] LB Monitor ID: %s", m.ID)

	log.Printf("[DEBUG] Waiting for OpenTelekomCloud LB Monitor (%s) to become available.", m.ID)
...
func resourceLBMonitorV1Read(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Retrieved OpenTelekomCloud LB Monitor %s: %+v", d.Id(), m)
...
func resourceLBMonitorV1Update(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Updating OpenTelekomCloud LB Monitor %s with options: %+v", d.Id(), updateOpts)

	_, err = monitors.Update(networkingClient, d.Id(), updateOpts).Extract()
	if err != nil {
		return fmt.Errorf("Error updating OpenTelekomCloud LB Monitor: %s", err)
	}
...
func resourceLBMonitorV1Delete(d *schema.ResourceData, meta interface{}) error {
	...
	_, err = stateConf.WaitForState()
	if err != nil {
		return fmt.Errorf("Error deleting OpenTelekomCloud LB Monitor: %s", err)
	}
...
func waitForLBMonitorActive(networkingClient *gophercloud.ServiceClient, monitorId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] OpenTelekomCloud LB Monitor: %+v", m)
		return m, "ACTIVE", nil
	}
}
...
func waitForLBMonitorDelete(networkingClient *gophercloud.ServiceClient, monitorId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		log.Printf("[DEBUG] Attempting to delete OpenTelekomCloud LB Monitor %s", monitorId)

		m, err := monitors.Get(networkingClient, monitorId).Extract()
		if err != nil {
			if _, ok := err.(gophercloud.ErrDefault404); ok {
				log.Printf("[DEBUG] Successfully deleted OpenTelekomCloud LB Monitor %s", monitorId)
				return m, "DELETED", nil
			}

			if errCode, ok := err.(gophercloud.ErrUnexpectedResponseCode); ok {
				if errCode.Actual == 409 {
					log.Printf("[DEBUG] OpenTelekomCloud LB Monitor (%s) is waiting for Pool to delete.", monitorId)
		...
		log.Printf("[DEBUG] OpenTelekomCloud LB Monitor %s still active.", monitorId)
		return m, "ACTIVE", nil
	}
