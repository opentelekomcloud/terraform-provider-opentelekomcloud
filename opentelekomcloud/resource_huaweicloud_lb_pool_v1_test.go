		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBV1PoolExists("opentelekomcloud_lb_pool_v1.pool_1", &pool),
					resource.TestCheckResourceAttr("opentelekomcloud_lb_pool_v1.pool_1", "lb_provider", "haproxy"),
				),
			},
			resource.TestStep{
				Config: testAccLBV1Pool_update,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("opentelekomcloud_lb_pool_v1.pool_1", "name", "pool_1"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckComputeV2InstanceExists("opentelekomcloud_compute_instance_v2.instance_1", &instance1),
					testAccCheckComputeV2InstanceExists("opentelekomcloud_compute_instance_v2.instance_2", &instance2),
					testAccCheckLBV1PoolExists("opentelekomcloud_lb_pool_v1.pool_1", &pool),
					testAccCheckLBV1MonitorExists("opentelekomcloud_lb_monitor_v1.monitor_1", &monitor),
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2SubnetExists("opentelekomcloud_networking_subnet_v2.subnet_1", &subnet),
					testAccCheckLBV1PoolExists("opentelekomcloud_lb_pool_v1.pool_1", &pool),
					testAccCheckLBV1MonitorExists("opentelekomcloud_lb_monitor_v1.monitor_1", &monitor_1),
					testAccCheckLBV1MonitorExists("opentelekomcloud_lb_monitor_v1.monitor_2", &monitor_2),
				),
			},
			resource.TestStep{
...
func testAccCheckLBV1PoolDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "opentelekomcloud_lb_pool_v1" {
			continue
		}
...
resource "opentelekomcloud_lb_monitor_v1" "monitor_2" {
  type = "TCP"
  delay = 30
  timeout = 5
  max_retries = 3
...
resource "opentelekomcloud_lb_pool_v1" "pool_1" {
  ...
  protocol = "TCP"
  lb_method = "ROUND_ROBIN"
  monitor_ids = ["${opentelekomcloud_lb_monitor_v1.monitor_2.id}"]
  subnet_id = "${opentelekomcloud_networking_subnet_v2.subnet_1.id}"
}
`
*/
