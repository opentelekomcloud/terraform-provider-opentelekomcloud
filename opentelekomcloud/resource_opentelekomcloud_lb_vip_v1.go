...
func resourceLBVipV1Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating OpenTelekomCloud LB VIP: %s", err)
	}
	log.Printf("[INFO] LB VIP ID: %s", p.ID)

	log.Printf("[DEBUG] Waiting for OpenTelekomCloud LB VIP (%s) to become available.", p.ID)
...
func resourceLBVipV1Read(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Retrieved OpenTelekomCloud LB VIP %s: %+v", d.Id(), p)
...
func resourceLBVipV1Update(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Updating OpenTelekomCloud LB VIP %s with options: %+v", d.Id(), updateOpts)

	_, err = vips.Update(networkingClient, d.Id(), updateOpts).Extract()
	if err != nil {
		return fmt.Errorf("Error updating OpenTelekomCloud LB VIP: %s", err)
	}
...
func resourceLBVipV1Delete(d *schema.ResourceData, meta interface{}) error {
	...
	_, err = stateConf.WaitForState()
	if err != nil {
		return fmt.Errorf("Error deleting OpenTelekomCloud LB VIP: %s", err)
	}
...
func waitForLBVIPActive(networkingClient *gophercloud.ServiceClient, vipId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] OpenTelekomCloud LB VIP: %+v", p)
		if p.Status == "ACTIVE" {
			return p, "ACTIVE", nil
		}
...
func waitForLBVIPDelete(networkingClient *gophercloud.ServiceClient, vipId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		log.Printf("[DEBUG] Attempting to delete OpenTelekomCloud LB VIP %s", vipId)

		p, err := vips.Get(networkingClient, vipId).Extract()
		if err != nil {
			if _, ok := err.(gophercloud.ErrDefault404); ok {
				log.Printf("[DEBUG] Successfully deleted OpenTelekomCloud LB VIP %s", vipId)
		...
		log.Printf("[DEBUG] OpenTelekomCloud LB VIP %s still active.", vipId)
		return p, "ACTIVE", nil
	}
