...
func TestAccCCENodePool_basic(t *testing.T) {
	...
	resourceName := "opentelekomcloud_cce_node_pool_v3.test"
	//clusterName here is used to provide the cluster id to fetch cce node pool.
	clusterName := "opentelekomcloud_cce_cluster_v3.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("opentelekomcloud_cce_cluster_v3.test", "name", "test"),
					...
					resource.TestCheckResourceAttr(resourceName, "max_node_count", "4"),
...
func testAccCheckCCENodePoolDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type == "opentelekomcloud_cce_cluster" {
			clusterId = rs.Primary.ID
		}

		if rs.Type == "opentelekomcloud_cce_node_pool" {
...
data "opentelekomcloud_availability_zones_v3" "test" {}

resource "opentelekomcloud_compute_keypair" "test" {
  name = "%s"
  public_key = "ssh-rsa  jrp-hp-pc"
}

resource "opentelekomcloud_cce_cluster_v3" "test" {
  name                   = "%s"
...
`, TestAccCCEClusterV3_basic(rName), rName, rName)
}
...
resource "opentelekomcloud_cce_node_pool_v3" "test" {
  cluster_id         = opentelekomcloud_cce_cluster.test.id
  name               = "%s"
  os                 = "EulerOS 2.5"
  flavor_id          = "s6.large.2"
  initial_node_count = 1
  availability_zone  = data.opentelekomcloud_availability_zones.test.names[0]
  key_pair           = opentelekomcloud_compute_keypair.test.name
  scall_enable      = false
  min_node_count    = 0
  max_node_count    = 0
