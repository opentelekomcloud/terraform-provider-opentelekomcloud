		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBV2ListenerExists("hwcloud_lb_listener_v2.listener_1", &listener),
					resource.TestCheckResourceAttr(
						"hwcloud_lb_listener_v2.listener_1", "connection_limit", "-1"),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"hwcloud_lb_listener_v2.listener_1", "name", "listener_1_updated"),
					resource.TestCheckResourceAttr(
						"hwcloud_lb_listener_v2.listener_1", "connection_limit", "100"),
				),
			},
		},
	})
...
func testAccCheckLBV2ListenerDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hwcloud_lb_listener_v2" {
			continue
		}
