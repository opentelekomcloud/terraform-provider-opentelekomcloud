...
func resourceDNSRecordSetV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HWCloud DNS record set: %s", err)
	}
	...
	log.Printf("[DEBUG] Created HWCloud DNS record set %s: %#v", n.ID, n)
...
func resourceDNSRecordSetV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error updating HWCloud DNS  record set: %s", err)
	}
...
func resourceDNSRecordSetV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error deleting HWCloud DNS  record set: %s", err)
	}
...
func waitForDNSRecordSet(dnsClient *gophercloud.ServiceClient, zoneID, recordsetId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] HWCloud DNS record set (%s) current status: %s", recordset.ID, recordset.Status)
