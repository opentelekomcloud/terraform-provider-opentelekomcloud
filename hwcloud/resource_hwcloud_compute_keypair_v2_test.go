		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2KeypairExists("hwcloud_compute_keypair_v2.kp_1", &keypair),
				),
			},
		},
	})
...
func testAccCheckComputeV2KeypairDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hwcloud_compute_keypair_v2" {
			continue
		}
...
resource "hwcloud_compute_keypair_v2" "kp_1" {
  name = "kp_1"
