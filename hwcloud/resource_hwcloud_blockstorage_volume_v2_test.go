		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckBlockStorageV2VolumeExists("hwcloud_blockstorage_volume_v2.volume_1", &volume),
					testAccCheckBlockStorageV2VolumeMetadata(&volume, "foo", "bar"),
					resource.TestCheckResourceAttr(
						"hwcloud_blockstorage_volume_v2.volume_1", "name", "volume_1"),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckBlockStorageV2VolumeMetadata(&volume, "foo", "bar"),
					resource.TestCheckResourceAttr(
						"hwcloud_blockstorage_volume_v2.volume_1", "name", "volume_1-updated"),
				),
			},
		},
	})
...
func testAccCheckBlockStorageV2VolumeDestroy(s *terraform.State) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HWCloud block storage client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hwcloud_blockstorage_volume_v2" {
			continue
		}
