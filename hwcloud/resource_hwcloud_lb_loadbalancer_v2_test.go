		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBV2LoadBalancerExists("hwcloud_lb_loadbalancer_v2.loadbalancer_1", &lb),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"hwcloud_lb_loadbalancer_v2.loadbalancer_1", "name", "loadbalancer_1_updated"),
					resource.TestMatchResourceAttr(
						"hwcloud_lb_loadbalancer_v2.loadbalancer_1", "vip_port_id",
			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBV2LoadBalancerExists(
						"hwcloud_lb_loadbalancer_v2.loadbalancer_1", &lb),
					testAccCheckNetworkingV2SecGroupExists(
						"hwcloud_networking_secgroup_v2.secgroup_1", &sg_1),
					testAccCheckNetworkingV2SecGroupExists(
						"hwcloud_networking_secgroup_v2.secgroup_1", &sg_2),
					resource.TestCheckResourceAttr(
						"hwcloud_lb_loadbalancer_v2.loadbalancer_1", "security_group_ids.#", "1"),
			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBV2LoadBalancerExists(
						"hwcloud_lb_loadbalancer_v2.loadbalancer_1", &lb),
					testAccCheckNetworkingV2SecGroupExists(
						"hwcloud_networking_secgroup_v2.secgroup_2", &sg_1),
					testAccCheckNetworkingV2SecGroupExists(
						"hwcloud_networking_secgroup_v2.secgroup_2", &sg_2),
					resource.TestCheckResourceAttr(
						"hwcloud_lb_loadbalancer_v2.loadbalancer_1", "security_group_ids.#", "2"),
					testAccCheckLBV2LoadBalancerHasSecGroup(&lb, &sg_1),
...
func testAccCheckLBV2LoadBalancerDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hwcloud_lb_loadbalancer_v2" {
			continue
		}
...
resource "hwcloud_lb_loadbalancer_v2" "loadbalancer_1" {
  name = "loadbalancer_1"
  loadbalancer_provider = "haproxy"
  vip_subnet_id = "${hwcloud_networking_subnet_v2.subnet_1.id}"
...
resource "hwcloud_networking_secgroup_v2" "secgroup_2" {
  name = "secgroup_2"
...
resource "hwcloud_lb_loadbalancer_v2" "loadbalancer_1" {
    ...
    vip_subnet_id = "${hwcloud_networking_subnet_v2.subnet_1.id}"
    security_group_ids = [
      "${hwcloud_networking_secgroup_v2.secgroup_1.id}"
    ]
}
`
...
resource "hwcloud_lb_loadbalancer_v2" "loadbalancer_1" {
    ...
    security_group_ids = [
      "${hwcloud_networking_secgroup_v2.secgroup_1.id}",
      "${hwcloud_networking_secgroup_v2.secgroup_2.id}"
    ]
}
`
...
resource "hwcloud_lb_loadbalancer_v2" "loadbalancer_1" {
    ...
    depends_on = ["hwcloud_networking_secgroup_v2.secgroup_1"]
}
`
