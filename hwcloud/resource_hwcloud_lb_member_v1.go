...
func resourceLBMemberV1Create(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] HWCloud LB Member Create Options: %#v", createOpts)
	m, err := members.Create(networkingClient, createOpts).Extract()
	if err != nil {
		return fmt.Errorf("Error creating HWCloud LB member: %s", err)
	}
	log.Printf("[INFO] LB member ID: %s", m.ID)

	log.Printf("[DEBUG] Waiting for HWCloud LB member (%s) to become available.", m.ID)
	...
	log.Printf("[DEBUG] HWCloud LB Member Update Options: %#v", createOpts)
	m, err = members.Update(networkingClient, m.ID, updateOpts).Extract()
	if err != nil {
		return fmt.Errorf("Error updating HWCloud LB member: %s", err)
	}
...
func resourceLBMemberV1Read(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Retrieved HWCloud LB member %s: %+v", d.Id(), m)
...
func resourceLBMemberV1Delete(d *schema.ResourceData, meta interface{}) error {
	...
	_, err = stateConf.WaitForState()
	if err != nil {
		return fmt.Errorf("Error deleting HWCloud LB member: %s", err)
	}
...
func waitForLBMemberActive(networkingClient *gophercloud.ServiceClient, memberId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] HWCloud LB member: %+v", m)
		if m.Status == "ACTIVE" {
			return m, "ACTIVE", nil
		}
...
func waitForLBMemberDelete(networkingClient *gophercloud.ServiceClient, memberId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		log.Printf("[DEBUG] Attempting to delete HWCloud LB member %s", memberId)

		m, err := members.Get(networkingClient, memberId).Extract()
		if err != nil {
			if _, ok := err.(gophercloud.ErrDefault404); ok {
				log.Printf("[DEBUG] Successfully deleted HWCloud LB member %s", memberId)
				return m, "DELETED", nil
			}
			return m, "ACTIVE", err
		}

		log.Printf("[DEBUG] HWCloud LB member %s still active.", memberId)
		return m, "ACTIVE", nil
	}
