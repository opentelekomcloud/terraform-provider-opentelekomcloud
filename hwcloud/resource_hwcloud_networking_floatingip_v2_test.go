...
func testAccCheckNetworkingV2FloatingIPDestroy(s *terraform.State) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HWCloud floating IP: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hwcloud_networking_floatingip_v2" {
			continue
		}
...
resource "hwcloud_networking_port_v2" "port_1" {
  admin_state_up = "true"
  network_id = "${hwcloud_networking_subnet_v2.subnet_1.network_id}"
...
resource "hwcloud_networking_floatingip_v2" "fip_1" {
  ...
  fixed_ip = "${hwcloud_networking_port_v2.port_1.fixed_ip.1.ip_address}"
}
