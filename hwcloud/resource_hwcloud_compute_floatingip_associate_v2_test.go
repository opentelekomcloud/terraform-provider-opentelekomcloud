		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2FloatingIPExists("hwcloud_networking_floatingip_v2.fip_1", &fip),
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2InstanceExists("hwcloud_compute_instance_v2.instance_1", &instance),
					testAccCheckNetworkingV2FloatingIPExists("hwcloud_networking_floatingip_v2.fip_1", &fip_1),
					testAccCheckNetworkingV2FloatingIPExists("hwcloud_networking_floatingip_v2.fip_2", &fip_2),
...
func testAccCheckComputeV2FloatingIPAssociateDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hwcloud_compute_floatingip_associate_v2" {
			continue
		}
...
resource "hwcloud_compute_floatingip_associate_v2" "fip_1" {
  floating_ip = "${hwcloud_networking_floatingip_v2.fip_1.address}"
  instance_id = "${hwcloud_compute_instance_v2.instance_1.id}"
  fixed_ip = "${hwcloud_compute_instance_v2.instance_1.access_ip_v4}"
}
`
...
resource "hwcloud_compute_floatingip_associate_v2" "fip_1" {
  floating_ip = "${hwcloud_networking_floatingip_v2.fip_1.address}"
  instance_id = "${hwcloud_compute_instance_v2.instance_1.id}"
  fixed_ip = "${hwcloud_compute_instance_v2.instance_1.network.0.fixed_ip_v4}"
}
`, OS_NETWORK_ID)
...
resource "hwcloud_networking_floatingip_v2" "fip_2" {
}
...
resource "hwcloud_compute_floatingip_associate_v2" "fip_1" {
  floating_ip = "${hwcloud_networking_floatingip_v2.fip_2.address}"
  instance_id = "${hwcloud_compute_instance_v2.instance_1.id}"
}
`
