		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWRuleV1Exists("hwcloud_fw_rule_v1.rule_1", rule1),
				),
			},
			resource.TestStep{
				Config: testAccFWRuleV1_basic_2,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWRuleV1Exists("hwcloud_fw_rule_v1.rule_1", rule2),
				),
			},
			resource.TestStep{
				Config: testAccFWRuleV1_basic_3,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWRuleV1Exists("hwcloud_fw_rule_v1.rule_1", rule3),
				),
			},
		},
	})
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWRuleV1Exists("hwcloud_fw_rule_v1.rule_1", rule),
				),
			},
		},
	})
...
func testAccCheckFWRuleV1Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hwcloud_firewall_rule" {
			continue
		}
