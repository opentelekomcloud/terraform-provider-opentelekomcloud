...
func resourceComputeSecGroupV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HWCloud security group: %s", err)
	}
	...
	for _, createRuleOpts := range createRuleOptsList {
		...
		if err != nil {
			return fmt.Errorf("Error creating HWCloud security group rule: %s", err)
		}
	}
...
func resourceComputeSecGroupV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error updating HWCloud security group (%s): %s", d.Id(), err)
	}
	...
	if d.HasChange("rule") {
		...
		for _, rawRule := range secgrouprulesToAdd.List() {
			...
			if err != nil {
				return fmt.Errorf("Error adding rule to HWCloud security group (%s): %s", d.Id(), err)
			}
			log.Printf("[DEBUG] Added rule (%s) to HWCloud security group (%s) ", rule.ID, d.Id())
		}
		...
		for _, r := range secgrouprulesToRemove.List() {
			...
			if err != nil {
				...
				return fmt.Errorf("Error removing rule (%s) from HWCloud security group (%s)", rule.ID, d.Id())
			} else {
				log.Printf("[DEBUG] Removed rule (%s) from HWCloud security group (%s): %s", rule.ID, d.Id(), err)
			}
		}
	}
...
func resourceComputeSecGroupV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error deleting HWCloud security group: %s", err)
	}
