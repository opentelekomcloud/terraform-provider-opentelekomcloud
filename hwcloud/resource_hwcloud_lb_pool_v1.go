	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"member": &schema.Schema{
				...
				Optional: true,
				Removed:  "Use hwcloud_lb_member_v1 instead.",
			},
...
func resourceLBPoolV1Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HWCloud LB pool: %s", err)
	}
	log.Printf("[INFO] LB Pool ID: %s", p.ID)

	log.Printf("[DEBUG] Waiting for HWCloud LB pool (%s) to become available.", p.ID)
	...
	if mIDs := resourcePoolMonitorIDsV1(d); mIDs != nil {
		for _, mID := range mIDs {
			...
			if err != nil {
				return fmt.Errorf("Error associating monitor (%s) with HWCloud LB pool (%s): %s", mID, p.ID, err)
			}
		}
	}
...
func resourceLBPoolV1Read(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Retrieved HWCloud LB Pool %s: %+v", d.Id(), p)
...
func resourceLBPoolV1Update(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Updating HWCloud LB Pool %s with options: %+v", d.Id(), updateOpts)

	_, err = pools.Update(networkingClient, d.Id(), updateOpts).Extract()
	if err != nil {
		return fmt.Errorf("Error updating HWCloud LB Pool: %s", err)
	}
	...
	if d.HasChange("monitor_ids") {
		...
		for _, m := range monitorsToAdd.List() {
			...
			if err != nil {
				return fmt.Errorf("Error associating monitor (%s) with HWCloud server (%s): %s", m.(string), d.Id(), err)
			}
		...
		for _, m := range monitorsToRemove.List() {
			...
			if err != nil {
				return fmt.Errorf("Error disassociating monitor (%s) from HWCloud server (%s): %s", m.(string), d.Id(), err)
			}
...
func resourceLBPoolV1Delete(d *schema.ResourceData, meta interface{}) error {
	...
	_, err = stateConf.WaitForState()
	if err != nil {
		return fmt.Errorf("Error deleting HWCloud LB Pool: %s", err)
	}
...
func waitForLBPoolActive(networkingClient *gophercloud.ServiceClient, poolId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] HWCloud LB Pool: %+v", p)
		if p.Status == "ACTIVE" {
			return p, "ACTIVE", nil
		}
...
func waitForLBPoolDelete(networkingClient *gophercloud.ServiceClient, poolId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		log.Printf("[DEBUG] Attempting to delete HWCloud LB Pool %s", poolId)

		p, err := pools.Get(networkingClient, poolId).Extract()
		if err != nil {
			if _, ok := err.(gophercloud.ErrDefault404); ok {
				log.Printf("[DEBUG] Successfully deleted HWCloud LB Pool %s", poolId)
		...
		log.Printf("[DEBUG] HWCloud LB Pool %s still active.", poolId)
		return p, "ACTIVE", nil
	}
