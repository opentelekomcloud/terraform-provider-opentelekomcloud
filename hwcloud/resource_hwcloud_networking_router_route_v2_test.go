			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2NetworkExists(
						"hwcloud_networking_network_v2.network_1", &network[0]),
					testAccCheckNetworkingV2SubnetExists(
						"hwcloud_networking_subnet_v2.subnet_1", &subnet[0]),
					testAccCheckNetworkingV2NetworkExists(
						"hwcloud_networking_network_v2.network_1", &network[1]),
					testAccCheckNetworkingV2SubnetExists(
						"hwcloud_networking_subnet_v2.subnet_1", &subnet[1]),
					testAccCheckNetworkingV2RouterInterfaceExists(
						"hwcloud_networking_router_interface_v2.int_1"),
					testAccCheckNetworkingV2RouterInterfaceExists(
						"hwcloud_networking_router_interface_v2.int_2"),
					testAccCheckNetworkingV2RouterRouteExists(
						"hwcloud_networking_router_route_v2.router_route_1"),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingV2RouterRouteExists(
						"hwcloud_networking_router_route_v2.router_route_1"),
					testAccCheckNetworkingV2RouterRouteExists(
						"hwcloud_networking_router_route_v2.router_route_2"),
				),
			},
			resource.TestStep{
				Config: testAccNetworkingV2RouterRoute_destroy,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingV2RouterRouteEmpty("hwcloud_networking_router_v2.router_1"),
				),
			},
		},
	})
...
resource "hwcloud_networking_router_interface_v2" "int_1" {
  ...
  port_id = "${hwcloud_networking_port_v2.port_1.id}"
}
...
resource "hwcloud_networking_router_interface_v2" "int_2" {
  router_id = "${hwcloud_networking_router_v2.router_1.id}"
...
resource "hwcloud_networking_router_route_v2" "router_route_2" {
  destination_cidr = "10.0.2.0/24"
  next_hop = "192.168.200.254"

  depends_on = ["hwcloud_networking_router_interface_v2.int_2"]
  router_id = "${hwcloud_networking_router_v2.router_1.id}"
}
`
