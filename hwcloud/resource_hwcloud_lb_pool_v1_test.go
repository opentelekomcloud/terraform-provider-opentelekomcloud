		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBV1PoolExists("hwcloud_lb_pool_v1.pool_1", &pool),
					resource.TestCheckResourceAttr("hwcloud_lb_pool_v1.pool_1", "lb_provider", "haproxy"),
				),
			},
			resource.TestStep{
				Config: testAccLBV1Pool_update,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("hwcloud_lb_pool_v1.pool_1", "name", "pool_1"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckComputeV2SecGroupExists("hwcloud_compute_secgroup_v2.secgroup_1", &secgroup),
					testAccCheckComputeV2InstanceExists("hwcloud_compute_instance_v2.instance_1", &instance1),
					testAccCheckComputeV2InstanceExists("hwcloud_compute_instance_v2.instance_2", &instance2),
					testAccCheckLBV1PoolExists("hwcloud_lb_pool_v1.pool_1", &pool),
					testAccCheckLBV1MonitorExists("hwcloud_lb_monitor_v1.monitor_1", &monitor),
					testAccCheckLBV1VIPExists("hwcloud_lb_vip_v1.vip_1", &vip),
				),
			},
			resource.TestStep{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2SubnetExists("hwcloud_networking_subnet_v2.subnet_1", &subnet),
					testAccCheckLBV1PoolExists("hwcloud_lb_pool_v1.pool_1", &pool),
					testAccCheckLBV1MonitorExists("hwcloud_lb_monitor_v1.monitor_1", &monitor_1),
					testAccCheckLBV1MonitorExists("hwcloud_lb_monitor_v1.monitor_2", &monitor_2),
				),
			},
			resource.TestStep{
...
resource "hwcloud_lb_monitor_v1" "monitor_2" {
  type = "TCP"
  delay = 30
  timeout = 5
  max_retries = 3
...
resource "hwcloud_lb_pool_v1" "pool_1" {
  ...
  protocol = "TCP"
  lb_method = "ROUND_ROBIN"
  monitor_ids = ["${hwcloud_lb_monitor_v1.monitor_2.id}"]
  subnet_id = "${hwcloud_networking_subnet_v2.subnet_1.id}"
}
`
*/
