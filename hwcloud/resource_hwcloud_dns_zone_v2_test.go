		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDNSV2ZoneExists("hwcloud_dns_zone_v2.zone_1", &zone),
					resource.TestCheckResourceAttr(
						"hwcloud_dns_zone_v2.zone_1", "description", "a zone"),
				),
			},
			resource.TestStep{
				Config: testAccDNSV2Zone_update(zoneName),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("hwcloud_dns_zone_v2.zone_1", "name", zoneName),
					resource.TestCheckResourceAttr("hwcloud_dns_zone_v2.zone_1", "email", "email2@example.com"),
					resource.TestCheckResourceAttr("hwcloud_dns_zone_v2.zone_1", "ttl", "6000"),
					resource.TestCheckResourceAttr("hwcloud_dns_zone_v2.zone_1", "type", "PRIMARY"),
					resource.TestCheckResourceAttr(
						"hwcloud_dns_zone_v2.zone_1", "description", "an updated zone"),
				),
			},
		},
	})
			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestMatchResourceAttr(
						"hwcloud_dns_zone_v2.zone_1", "ttl", regexp.MustCompile("^[0-9]+$")),
				),
			},
		},
	})
...
func testAccCheckDNSV2ZoneDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hwcloud_dns_zone_v2" {
			continue
		}
