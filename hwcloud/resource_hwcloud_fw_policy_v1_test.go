			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWPolicyV1Exists(
						"hwcloud_fw_policy_v1.policy_1", "", "", 0),
				),
			},
		},
	})
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWPolicyV1Exists(
						"hwcloud_fw_policy_v1.policy_1", "policy_1", "terraform acceptance test", 2),
				),
			},
		},
	})
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWPolicyV1Exists(
						"hwcloud_fw_policy_v1.policy_1", "policy_1", "terraform acceptance test", 1),
				),
			},
		},
	})
...
func testAccCheckFWPolicyV1Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hwcloud_fw_policy_v1" {
			continue
		}
...
resource "hwcloud_fw_policy_v1" "policy_1" {
  ...
  rules = [
    "${hwcloud_fw_rule_v1.udp_deny.id}",
    "${hwcloud_fw_rule_v1.tcp_allow.id}"
  ]
}

resource "hwcloud_fw_rule_v1" "tcp_allow" {
  protocol = "tcp"
  action = "allow"
}

resource "hwcloud_fw_rule_v1" "udp_deny" {
  protocol = "udp"
  action = "deny"
}
`
