			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrPtr(
						"hwcloud_networking_secgroup_v2.secgroup_1", "id", &security_group.ID),
					resource.TestCheckResourceAttr(
						"hwcloud_networking_secgroup_v2.secgroup_1", "name", "security_group_2"),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2SecGroupDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hwcloud_networking_secgroup_v2" {
			continue
		}
