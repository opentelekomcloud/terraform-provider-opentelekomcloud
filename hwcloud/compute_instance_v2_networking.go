...
// on an hwcloud_compute_instance_v2 resource.
//
...
// can be consumed by both HWCloud and Terraform.
//
...
// returned by an HWCloud instance only has the network name set! So if a
// user specified a network ID, there's no way to correlate it to the instance
// unless we know both the name and ID.
//
// Not only that, but we have to account for two HWCloud network services
...
func getInstanceNetworkInfo(
	...
	if err != nil {
		return nil, fmt.Errorf("Error creating HWCloud compute client: %s", err)
	}
...
func getInstanceAccessAddresses(
	...
	log.Printf("[DEBUG] HWCloud Instance Network Access Addresses: %s, %s", hostv4, hostv6)
