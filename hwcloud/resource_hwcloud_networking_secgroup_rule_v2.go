...
func resourceNetworkingSecGroupRuleV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] HWCloud Neutron Security Group Rule created: %#v", security_group_rule)
...
func resourceNetworkingSecGroupRuleV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return CheckDeleted(d, err, "HWCloud Security Group Rule")
	}
...
func resourceNetworkingSecGroupRuleV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error deleting HWCloud Neutron Security Group Rule: %s", err)
	}
...
func waitForSecGroupRuleDelete(networkingClient *gophercloud.ServiceClient, secGroupRuleId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		log.Printf("[DEBUG] Attempting to delete HWCloud Security Group Rule %s.\n", secGroupRuleId)

		r, err := rules.Get(networkingClient, secGroupRuleId).Extract()
		if err != nil {
			if _, ok := err.(gophercloud.ErrDefault404); ok {
				log.Printf("[DEBUG] Successfully deleted HWCloud Neutron Security Group Rule %s", secGroupRuleId)
		...
		log.Printf("[DEBUG] HWCloud Neutron Security Group Rule %s still active.\n", secGroupRuleId)
		return r, "ACTIVE", nil
	}
}
