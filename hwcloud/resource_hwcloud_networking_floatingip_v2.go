...
func resourceNetworkFloatingIPV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HWCloud network client: %s", err)
	}
	...
	log.Printf("[DEBUG] Waiting for HWCloud Neutron Floating IP (%s) to become available.", floatingIP.ID)
...
func resourceNetworkFloatingIPV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	_, err = stateConf.WaitForState()
	if err != nil {
		return fmt.Errorf("Error deleting HWCloud Neutron Floating IP: %s", err)
	}
...
func waitForFloatingIPActive(networkingClient *gophercloud.ServiceClient, fId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] HWCloud Neutron Floating IP: %+v", f)
...
func waitForFloatingIPDelete(networkingClient *gophercloud.ServiceClient, fId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		log.Printf("[DEBUG] Attempting to delete HWCloud Floating IP %s.\n", fId)

		f, err := floatingips.Get(networkingClient, fId).Extract()
		if err != nil {
			if _, ok := err.(gophercloud.ErrDefault404); ok {
				log.Printf("[DEBUG] Successfully deleted HWCloud Floating IP %s", fId)
		...
		log.Printf("[DEBUG] HWCloud Floating IP %s still active.\n", fId)
		return f, "ACTIVE", nil
	}
}
