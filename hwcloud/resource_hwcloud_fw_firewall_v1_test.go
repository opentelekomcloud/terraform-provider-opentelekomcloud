		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWFirewallV1("hwcloud_fw_firewall_v1.fw_1", "", "", policyID),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWFirewallV1(
						"hwcloud_fw_firewall_v1.fw_1", "fw_1", "terraform acceptance test", policyID),
				),
			},
		},
	})
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWFirewallV1Exists("hwcloud_fw_firewall_v1.fw_1", &firewall),
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWFirewallV1Exists("hwcloud_fw_firewall_v1.fw_1", &firewall),
					resource.TestCheckResourceAttr("hwcloud_fw_firewall_v1.fw_1", "description", "firewall router test"),
...
func testAccCheckFWFirewallV1Exists(n string, firewall *Firewall) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		networkingClient, err := config.networkingV2Client(OS_REGION_NAME)
		if err != nil {
			return fmt.Errorf("Exists) Error creating HWCloud networking client: %s", err)
		}
...
resource "hwcloud_fw_firewall_v1" "fw_1" {
  policy_id = "${hwcloud_fw_policy_v1.policy_1.id}"
...
resource "hwcloud_fw_firewall_v1" "fw_1" {
  ...
  policy_id = "${hwcloud_fw_policy_v1.policy_2.id}"
  admin_state_up = true
...
resource "hwcloud_fw_policy_v1" "policy_2" {
  name = "policy_2"
}
`
...
resource "hwcloud_fw_firewall_v1" "fw_1" {
  ...
  policy_id = "${hwcloud_fw_policy_v1.policy_1.id}"
  associated_routers = ["${hwcloud_networking_router_v2.router_1.id}"]
}
`
...
resource "hwcloud_networking_router_v2" "router_2" {
  name = "router_2"
  admin_state_up = "true"
  distributed = "false"
}
...
resource "hwcloud_fw_firewall_v1" "fw_1" {
  ...
  policy_id = "${hwcloud_fw_policy_v1.policy_1.id}"
  associated_routers = [
    "${hwcloud_networking_router_v2.router_1.id}",
    "${hwcloud_networking_router_v2.router_2.id}"
  ]
}
`
