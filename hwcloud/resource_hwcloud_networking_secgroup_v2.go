...
func resourceNetworkingSecGroupV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Create HWCloud Neutron Security Group: %#v", opts)
	...
	log.Printf("[DEBUG] HWCloud Neutron Security Group created: %#v", security_group)
...
func resourceNetworkingSecGroupV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return CheckDeleted(d, err, "HWCloud Neutron Security group")
	}
...
func resourceNetworkingSecGroupV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if update {
		...
		if err != nil {
			return fmt.Errorf("Error updating HWCloud SecGroup: %s", err)
		}
	}
...
func resourceNetworkingSecGroupV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error deleting HWCloud Neutron Security Group: %s", err)
	}
...
func waitForSecGroupDelete(networkingClient *gophercloud.ServiceClient, secGroupId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		log.Printf("[DEBUG] Attempting to delete HWCloud Security Group %s.\n", secGroupId)

		r, err := groups.Get(networkingClient, secGroupId).Extract()
		if err != nil {
			if _, ok := err.(gophercloud.ErrDefault404); ok {
				log.Printf("[DEBUG] Successfully deleted HWCloud Neutron Security Group %s", secGroupId)
		...
		log.Printf("[DEBUG] HWCloud Neutron Security Group %s still active.\n", secGroupId)
		return r, "ACTIVE", nil
	}
}
