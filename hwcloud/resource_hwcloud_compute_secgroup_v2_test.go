		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2SecGroupExists("hwcloud_compute_secgroup_v2.sg_1", &secgroup),
				),
			},
		},
	})
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2SecGroupExists("hwcloud_compute_secgroup_v2.sg_1", &secgroup1),
					testAccCheckComputeV2SecGroupExists("hwcloud_compute_secgroup_v2.sg_2", &secgroup2),
					testAccCheckComputeV2SecGroupExists("hwcloud_compute_secgroup_v2.sg_3", &secgroup3),
			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						"hwcloud_compute_secgroup_v2.sg_1", "rule.3170486100.self", "true"),
					resource.TestCheckResourceAttr(
						"hwcloud_compute_secgroup_v2.sg_1", "rule.3170486100.from_group_id", ""),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2SecGroupExists("hwcloud_compute_secgroup_v2.sg_1", &secgroup),
					resource.TestCheckResourceAttr(
						"hwcloud_compute_secgroup_v2.sg_1", "rule.3862435458.cidr", "2001:558:fc00::/39"),
				),
			},
		},
	})
...
func testAccCheckComputeV2SecGroupDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hwcloud_compute_secgroup_v2" {
			continue
		}
...
resource "hwcloud_compute_secgroup_v2" "sg_1" {
  name = "sg_1"
...
resource "hwcloud_compute_secgroup_v2" "sg_2" {
  name = "sg_2"
...
resource "hwcloud_compute_secgroup_v2" "sg_3" {
  name = "sg_3"
  ...
  rule {
    ...
    to_port = 80
    ip_protocol = "tcp"
    from_group_id = "${hwcloud_compute_secgroup_v2.sg_1.id}"
  }
}
`
...
resource "hwcloud_compute_secgroup_v2" "sg_3" {
  ...
  rule {
    ...
    to_port = 80
    ip_protocol = "tcp"
    from_group_id = "${hwcloud_compute_secgroup_v2.sg_2.id}"
  }
}
`
