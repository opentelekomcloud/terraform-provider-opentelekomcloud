	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"floating_ip": &schema.Schema{
				...
				Optional: true,
				ForceNew: false,
				Removed:  "Use the hwcloud_compute_floatingip_associate_v2 resource instead",
			},
			...
			"volume": &schema.Schema{
				Type:     schema.TypeSet,
				Optional: true,
				Removed:  "Use block_device or hwcloud_compute_volume_attach_v2 instead",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"id": &schema.Schema{
							Type:     schema.TypeString,
...
func resourceComputeInstanceV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HWCloud server: %s", err)
	}
...
func resourceComputeInstanceV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	// AccessIPv4/v6 isn't standard in HWCloud, but there have been reports
	...
	if !ok {
		return fmt.Errorf("Error setting HWCloud server's flavor: %v", server.Flavor)
	}
...
func resourceComputeInstanceV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if updateOpts != (servers.UpdateOpts{}) {
		...
		if err != nil {
			return fmt.Errorf("Error updating HWCloud server: %s", err)
		}
	}
	...
	if d.HasChange("metadata") {
		...
		if err != nil {
			return fmt.Errorf("Error updating HWCloud server (%s) metadata: %s", d.Id(), err)
		}
	}
	...
	if d.HasChange("security_groups") {
		...
		for _, g := range secgroupsToRemove.List() {
			...
			if err != nil && err.Error() != "EOF" {
				...
				return fmt.Errorf("Error removing security group (%s) from HWCloud server (%s): %s", g, d.Id(), err)
			} else {
		...
		for _, g := range secgroupsToAdd.List() {
			...
			if err != nil && err.Error() != "EOF" {
				return fmt.Errorf("Error adding security group (%s) to HWCloud server (%s): %s", g, d.Id(), err)
			}
	...
	if d.HasChange("admin_pass") {
		if newPwd, ok := d.Get("admin_pass").(string); ok {
			...
			if err != nil {
				return fmt.Errorf("Error changing admin password of HWCloud server (%s): %s", d.Id(), err)
			}
		}
	}
	...
	if d.HasChange("flavor_id") || d.HasChange("flavor_name") {
		...
		if err != nil {
			return fmt.Errorf("Error resizing HWCloud server: %s", err)
		}
		...
		if err != nil {
			return fmt.Errorf("Error confirming resize of HWCloud server: %s", err)
		}
...
func resourceComputeInstanceV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if d.Get("stop_before_destroy").(bool) {
		...
		if err != nil {
			log.Printf("[WARN] Error stopping HWCloud instance: %s", err)
		} else {
	...
	if d.Get("force_delete").(bool) {
		log.Printf("[DEBUG] Force deleting HWCloud Instance %s", d.Id())
		err = servers.ForceDelete(computeClient, d.Id()).ExtractErr()
		if err != nil {
			return fmt.Errorf("Error deleting HWCloud server: %s", err)
		}
	} else {
		log.Printf("[DEBUG] Deleting HWCloud Instance %s", d.Id())
	...
	if v := d.Get("personality"); v != nil {
		...
		if len(personalityList) > 0 {
			for _, p := range personalityList {
				...
				log.Printf("[DEBUG] HWCloud Compute Instance Personality: %+v", file)
