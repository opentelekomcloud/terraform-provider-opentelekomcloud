		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2SecGroupExists(
						"hwcloud_networking_secgroup_v2.secgroup_2", &secgroup_2),
					testAccCheckNetworkingV2SecGroupRuleExists(
						"hwcloud_networking_secgroup_rule_v2.secgroup_rule_1", &secgroup_rule_1),
					testAccCheckNetworkingV2SecGroupRuleExists(
						"hwcloud_networking_secgroup_rule_v2.secgroup_rule_2", &secgroup_rule_2),
				),
			},
		},
	})
			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						"hwcloud_networking_secgroup_rule_v2.secgroup_rule_1", "remote_ip_prefix", "2001:558:fc00::/39"),
				),
			},
		},
	})
			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2SecGroupRuleExists(
						"hwcloud_networking_secgroup_rule_v2.secgroup_rule_1", &secgroup_rule_1),
					resource.TestCheckResourceAttr(
						"hwcloud_networking_secgroup_rule_v2.secgroup_rule_1", "protocol", "115"),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2SecGroupRuleDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hwcloud_networking_secgroup_rule_v2" {
			continue
		}
...
resource "hwcloud_networking_secgroup_rule_v2" "secgroup_rule_1" {
  direction = "ingress"
  ethertype = "IPv4"
  port_range_max = 22
  port_range_min = 22
...
resource "hwcloud_networking_secgroup_rule_v2" "secgroup_rule_2" {
  direction = "ingress"
  ethertype = "IPv4"
  port_range_max = 80
  port_range_min = 80
  protocol = "tcp"
  remote_group_id = "${hwcloud_networking_secgroup_v2.secgroup_1.id}"
  security_group_id = "${hwcloud_networking_secgroup_v2.secgroup_2.id}"
}
`
