		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBV2MemberExists("hwcloud_lb_member_v2.member_1", &member_1),
					testAccCheckLBV2MemberExists("hwcloud_lb_member_v2.member_2", &member_2),
				),
			},
			resource.TestStep{
				Config: TestAccLBV2MemberConfig_update,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("hwcloud_lb_member_v2.member_1", "weight", "10"),
					resource.TestCheckResourceAttr("hwcloud_lb_member_v2.member_2", "weight", "15"),
				),
			},
		},
	})
...
func testAccCheckLBV2MemberDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hwcloud_lb_member_v2" {
			continue
		}
...
resource "hwcloud_lb_listener_v2" "listener_1" {
  name = "listener_1"
  protocol = "HTTP"
  protocol_port = 8080
  loadbalancer_id = "${hwcloud_lb_loadbalancer_v2.loadbalancer_1.id}"
}

resource "hwcloud_lb_pool_v2" "pool_1" {
  name = "pool_1"
  protocol = "HTTP"
  lb_method = "ROUND_ROBIN"
  listener_id = "${hwcloud_lb_listener_v2.listener_1.id}"
}
...
resource "hwcloud_lb_member_v2" "member_2" {
