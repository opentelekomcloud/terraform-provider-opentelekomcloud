		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBV2MonitorExists(t, "hwcloud_lb_monitor_v2.monitor_1", &monitor),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"hwcloud_lb_monitor_v2.monitor_1", "name", "monitor_1_updated"),
					resource.TestCheckResourceAttr("hwcloud_lb_monitor_v2.monitor_1", "delay", "30"),
					resource.TestCheckResourceAttr("hwcloud_lb_monitor_v2.monitor_1", "timeout", "15"),
				),
			},
		},
	})
...
func testAccCheckLBV2MonitorDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hwcloud_lb_monitor_v2" {
			continue
		}
