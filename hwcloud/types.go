...
func (lrt *LogRoundTripper) RoundTrip(request *http.Request) (*http.Response, error) {
	...
	if lrt.OsDebug {
		log.Printf("[DEBUG] HWCloud Request URL: %s %s", request.Method, request.URL)
...
func (lrt *LogRoundTripper) logRequest(original io.ReadCloser, contentType string) (io.ReadCloser, error) {
	...
	if strings.HasPrefix(contentType, "application/json") {
		...
		log.Printf("[DEBUG] HWCloud Request Body: %s", debugInfo)
	} else {
		log.Printf("[DEBUG] HWCloud Request Body: %s", bs.String())
	}
...
func (lrt *LogRoundTripper) logResponse(original io.ReadCloser, contentType string) (io.ReadCloser, error) {
	if strings.HasPrefix(contentType, "application/json") {
		...
		debugInfo := lrt.formatJSON(bs.Bytes())
		if debugInfo != "" {
			log.Printf("[DEBUG] HWCloud Response Body: %s", debugInfo)
		}
		return ioutil.NopCloser(strings.NewReader(bs.String())), nil
	}

	log.Printf("[DEBUG] Not logging because HWCloud response body isn't JSON")
	return original, nil
}
...
func (lrt *LogRoundTripper) formatJSON(raw []byte) string {
	...
	if err != nil {
		log.Printf("[DEBUG] Unable to parse HWCloud JSON: %s", err)
		return string(raw)
	}
	...
	if err != nil {
		log.Printf("[DEBUG] Unable to re-marshal HWCloud JSON: %s", err)
		return string(raw)
	}
...
// Firewall is an HWCloud firewall.
