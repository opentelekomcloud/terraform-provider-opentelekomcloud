...
func resourceNetworkingRouterInterfaceV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HWCloud Neutron router interface: %s", err)
	}
...
func resourceNetworkingRouterInterfaceV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		...
		return fmt.Errorf("Error retrieving HWCloud Neutron Router Interface: %s", err)
	}
...
func resourceNetworkingRouterInterfaceV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	_, err = stateConf.WaitForState()
	if err != nil {
		return fmt.Errorf("Error deleting HWCloud Neutron Router Interface: %s", err)
	}
...
func waitForRouterInterfaceActive(networkingClient *gophercloud.ServiceClient, rId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] HWCloud Neutron Router Interface: %+v", r)
...
func waitForRouterInterfaceDelete(networkingClient *gophercloud.ServiceClient, d *schema.ResourceData) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] Attempting to delete HWCloud Router Interface %s.", routerInterfaceId)
		...
		if err != nil {
			if _, ok := err.(gophercloud.ErrDefault404); ok {
				log.Printf("[DEBUG] Successfully deleted HWCloud Router Interface %s", routerInterfaceId)
		...
		log.Printf("[DEBUG] HWCloud Router Interface %s is still active.", routerInterfaceId)
		return r, "ACTIVE", nil
	}
}
