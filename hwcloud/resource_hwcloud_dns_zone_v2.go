...
func resourceDNSZoneV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HWCloud DNS zone: %s", err)
	}
	...
	log.Printf("[DEBUG] Created HWCloud DNS Zone %s: %#v", n.ID, n)
...
func resourceDNSZoneV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error updating HWCloud DNS Zone: %s", err)
	}
...
func resourceDNSZoneV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error deleting HWCloud DNS Zone: %s", err)
	}
...
func waitForDNSZone(dnsClient *gophercloud.ServiceClient, zoneId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] HWCloud DNS Zone (%s) current status: %s", zone.ID, zone.Status)
