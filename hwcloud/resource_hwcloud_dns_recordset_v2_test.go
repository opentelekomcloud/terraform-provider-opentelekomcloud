		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDNSV2RecordSetExists("hwcloud_dns_recordset_v2.recordset_1", &recordset),
					resource.TestCheckResourceAttr(
						"hwcloud_dns_recordset_v2.recordset_1", "description", "a record set"),
					resource.TestCheckResourceAttr(
						"hwcloud_dns_recordset_v2.recordset_1", "records.0", "10.1.0.0"),
				),
			},
			resource.TestStep{
				Config: testAccDNSV2RecordSet_update(zoneName),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("hwcloud_dns_recordset_v2.recordset_1", "name", zoneName),
					resource.TestCheckResourceAttr("hwcloud_dns_recordset_v2.recordset_1", "ttl", "6000"),
					resource.TestCheckResourceAttr("hwcloud_dns_recordset_v2.recordset_1", "type", "A"),
					resource.TestCheckResourceAttr(
						"hwcloud_dns_recordset_v2.recordset_1", "description", "an updated record set"),
					resource.TestCheckResourceAttr(
						"hwcloud_dns_recordset_v2.recordset_1", "records.0", "10.1.0.1"),
				),
			},
		},
	})
			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestMatchResourceAttr(
						"hwcloud_dns_recordset_v2.recordset_1", "ttl", regexp.MustCompile("^[0-9]+$")),
				),
			},
		},
	})
...
func testAccCheckDNSV2RecordSetDestroy(s *terraform.State) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HWCloud DNS client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hwcloud_dns_recordset_v2" {
			continue
		}
...
func testAccDNSV2RecordSet_basic(zoneName string) string {
	return fmt.Sprintf(`
		resource "hwcloud_dns_zone_v2" "zone_1" {
			name = "%s"
		...
		resource "hwcloud_dns_recordset_v2" "recordset_1" {
			zone_id = "${hwcloud_dns_zone_v2.zone_1.id}"
			name = "%s"
			type = "A"
