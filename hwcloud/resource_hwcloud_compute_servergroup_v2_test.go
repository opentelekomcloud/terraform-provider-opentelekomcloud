		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2ServerGroupExists("hwcloud_compute_servergroup_v2.sg_1", &sg),
				),
			},
		},
	})
...
func testAccCheckComputeV2ServerGroupDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hwcloud_compute_servergroup_v2" {
			continue
		}
...
resource "hwcloud_compute_servergroup_v2" "sg_1" {
  name = "sg_1"
  policies = ["affinity"]
}
`
